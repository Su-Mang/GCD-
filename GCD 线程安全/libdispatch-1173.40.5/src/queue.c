...
__DISPATCH_WAIT_FOR_QUEUE__(dispatch_sync_context_t dsc, dispatch_queue_t dq)
{
	...
	// 死锁  "dispatch_sync called on queue ""already owned by current thread"
...
_dispatch_barrier_sync_f_inline(dispatch_queue_t dq, void *ctxt,
		dispatch_function_t func, uintptr_t dc_flags)
{    // 获取线程  pthread_mach_thread_np(_dispatch_thread_self())
	//
	dispatch_tid tid = _dispatch_tid_self();
		...
		dispatch_queue_t tq, bool legacy)
{
	...
	if (!tq) {
		tq = _dispatch_get_root_queue(
				qos == DISPATCH_QOS_UNSPECIFIED ? DISPATCH_QOS_DEFAULT : qos, //4|0
				overcommit == _dispatch_queu e_attr_overcommit_enabled)->_as_dq;// 0|1
		if (unlikely(!tq)) {
	...
	//开辟内存生成相应的queue
	dispatch_lan e_t dq = _dispatch_object_alloc(vtable,
...
{
	return _dispatch_lane_create _with_target(label, attr,
			DISPA TCH_TARGET_QUEUE_DEFAULT, true);
}
